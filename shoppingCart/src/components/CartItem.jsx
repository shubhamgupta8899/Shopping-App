...
const CartItem = ({ item }) => {
  ...
  return (
    <div
      className=" flex flex-col items-center justify-between sm:flex-row p-4 sm:p-6 mb-4 bg-gray-900 rounded-xl
    ...
    >
      <div className="flex items-center space-x-4 w-full sm:w-auto">
        ...
        />
        <div className="grow">
          <h3 className="text-sm lg:text-xl md:text-xl font-bold text-white line-clamp-1">
            {item.name}
          </h3>
      ...
      <div className="flex items-center justify-between sm:justify-end w-full sm:w-2/5 sm:mt-0 space-x-4">
        <div className="flex items-center border border-gray-700 rounded-full overflow-hidden shadow-lg">
          <button
            ...
            className="p-2 text-gray-400 bg-gray-800 hover:bg-gray-700 transition duration-150
          w-8 h-8 flex items-center justify-center"
          >
            -
          </button>
          <span className="px-3 bg-gray-800 text-white font-bold text-base">
            {item.quantity}
          </span>
          <button
        ...
        <button onClick={() => removeFromCart(item.id, true)}
          className="p-3 bg-red-800/20 text-red-400 transition hover:bg-red-700/50 rounded-full">
          <X className="w-5 h-5" />
        </button>
      </div>
    </div>
